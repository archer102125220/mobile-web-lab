...
└── navigator-language/       # [實驗 3] 跨平台語系與 Locale 判斷測試 (籌備中)
```
...
| **`navigator-language`** *(籌備中)* | 🤖 Android<br>🍏 iOS<br>🌐 Web | • 系統設定語系 vs. 瀏覽器 App 語系 vs. WebView 語系<br>• `navigator.language` / `navigator.languages` 陣列回傳順序<br>• HTTP `Accept-Language` 標頭一致性與解析規則 | 規劃中 |
