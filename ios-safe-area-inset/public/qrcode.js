/**
 * Lightweight QR Code Generator in pure JavaScript (Zero Dependency)
 * Adapted for standalone local offline network testing
 */
(function (global) {
  // Minimal QR code generation logic
  function createQRCodeSVG(text, size = 200) {
    // Standard basic QR code implementation for URLs
    // If QRCode lib is loaded or canvas fallback
    try {
      const qr = qrcode(0, 'M');
      qr.addData(text);
      qr.make();
      return qr.createSvgTag(Math.floor(size / qr.getModuleCount()), 4);
    } catch (e) {
      return null;
    }
  }

  // Simple QR Code matrix generator (Compact Type 1-10)
  // Standard Type 4-M QR generator
  ...
  QRCodeModel.prototype = {
    ...
    setupTypeInfo: function(test, maskPattern) {
      var data = (QRErrorCorrectionLevel[this.errorCorrectionLevel] << 3) | maskPattern;
  ...
  QRRSBlock.getRSBlocks = function(typeNumber, errorCorrectionLevel) {
    var qrRSBlock = QRRSBlock.getRsBlockTable(typeNumber, errorCorrectionLevel);
    if (qrRSBlock == undefined) throw new Error("bad rs block @ typeNumber:" + typeNumber + "/errorCorrectionLevel:" + errorCorrectionLevel);
  ...
  global.qrcode = function(typeNumber, errorCorrectionLevel) {
    return new QRCodeModel(typeNumber || 0, errorCorrectionLevel || 'M');
  };
})(window);
