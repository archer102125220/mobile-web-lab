...
Android 平台一直以來以「碎片化 (Fragmentation)」聞名。然而在探討 WebView 的彈窗與「User Activation v2 (UAv2)」機制時，其影響並非單純取決於手機硬體廠牌，而是取決於**底層瀏覽器內核的來源、版本以及 Android WebViewClient 原生架構限制**。

> [!NOTE]
> **📱 實測驗證基準環境 (Benchmark Environment)**
> - 🤖 **Android 設備**：Samsung Galaxy Z Fold5 (Android 16 / One UI 8.5 / Chromium WebView)

---

## 1. 具備 GMS 的一般廠牌：高度一致的 Chromium 體驗與 UAv2

自從 Android 5.0 (Lollipop) 之後，Google 將 Android System WebView 從作業系統底層抽離，改為透過 Google Play Store 獨立發布與更新。

- **一致的 UAv2 機制**：無論是 Samsung 的 One UI、Xiaomi 的 HyperOS、或是 Pixel 等系統，只要設備搭載 Google Mobile Services (GMS) 且有定期更新，底層運作的皆為官方最新版 Chromium 引擎。
- **測試結果通用與 5 秒寬限期**：在這些設備上，Chromium UAv2 的「5 秒鐘憑證寬限期 (`kActivationLifespan`)」機制會高度一致地嚴格運作。只要非同步延遲在最後一次互動後的 5 秒內，`window.open` 或動態觸發的 `<a target="_blank">` 皆能順利被原生層捕捉並放行；一旦完全無互動且延遲超過 5 秒，憑證過期即遭 Popup Blocker 攔截。
- **憑證刷新機制 (Token Refresh)**：這 5 秒並非從第一次點擊死死綁定。Chromium 底層具備刷新機制——只要在 5 秒倒數期間內，使用者**持續與畫面產生任何有效互動（例如滑動螢幕、觸碰、再次點擊）**，這個 5 秒的計時器就會被**重置刷新**。因此即使非同步 API 回應時間較長，只要使用者在此期間有持續操作畫面，手勢憑證依然能持續續期並放行彈窗。

---

## 2. 原生架構盲區：Android `shouldOverrideUrlLoading` 永遠不攔截 POST

在本次實測（Samsung Galaxy Z Fold5 / Android 16）中，最關鍵的原生架構限制在於 **表單 POST 跳轉**：
- **官方規範與 AOSP 原始碼明確註記**：在 [Android 官方開發者文檔 (String url 多載)](https://developer.android.com/reference/android/webkit/WebViewClient#shouldOverrideUrlLoading(android.webkit.WebView,%20java.lang.String)) 以及 [AOSP 官方原始碼 (WebViewClient.java)](https://android.googlesource.com/platform/frameworks/base/+/master/core/java/android/webkit/WebViewClient.java) 中均明確標記：`"Note: this method is not called for POST requests."`。
- **現代 Chromium 底層行為一致**：雖然 API 24 新增了 `shouldOverrideUrlLoading(WebView, WebResourceRequest)` 多載，但底層 Chromium 內核（`InterceptNavigationDelegate`）依然沿用這套設計原則——`shouldOverrideUrlLoading` 是專門設計給 URL 導航（GET 請求）進行攔截的；針對帶有 Payload 的 POST 表單請求，系統不會觸發該攔截器，若需要監聽或攔截 POST 請求，官方指引要求改用更底層的 `shouldInterceptRequest`。
- **原頁 POST 穿透**：當網頁提交 POST `target="_self"` 表單時，`shouldOverrideUrlLoading` 永不回呼，請求直接在 WebView 內部穿透載入。
- **新視窗 POST `_blank` 靜默無反應**：當提交 POST `target="_blank"` 表單時，雖然觸發了 `WebChromeClient.onCreateWindow`，但新視窗中的 `WebViewClient.shouldOverrideUrlLoading` 依舊不處理 POST 請求；若宿主 App 未將臨時 `WebView` 實體貼上畫面，該請求將在背景無聲無息地消失，造成 UI 上完全靜默。

---

## 3. 真正的碎片化地雷一：超級 App 的第三方內核 (如騰訊 X5)
...
## 4. 真正的碎片化地雷二：無 Google 服務的手機 (如華為 HarmonyOS)
...
## 6. 跨平台的共同死穴：App 拒絕實作 `WebChromeClient`
...
## 7. 附錄：Android WebView 版本與支援生命週期 (紀錄於 2026-09-04)

在規劃跨平台 WebView 開發時，了解 Android WebView 的演進與支援界線是非常重要的：

- **獨立更新的起點**：Google 於 **Android 5.0 (Lollipop, 2014年)** 將 WebView 從作業系統核心中抽離，改為透過 Google Play Store 更新。這解決了以往「系統不更新，WebView 就永遠無法升級」的災難。
...
- **官方明定的版本支援**：截至 **2026 年 9 月**，多數現代化與主流 App 的最低支援版本通常設定在 **Android 8.0 (API 26)** 或 **Android 9.0 (API 28)**。
  - **選定 Android 8.0 (2017年8月發布) 的主因**：從此版本開始，WebView 預設啟用**多程序架構 (Multiprocess)**，網頁崩潰不再容易導致整個 App 閃退。
  - **選定 Android 9.0 (2018年8月發布) 的主因**：此版本開始**預設值改為封鎖 HTTP (強制 HTTPS)**，且正式引入**瀏海螢幕 (Display Cutout) API**。
- **最新測試基準**：本專案測試設備 **Samsung Galaxy Z Fold5** 搭載 **One UI 8.5 (Android 16)**，採用現代 Chromium 引擎，具備完整的 UAv2 支援。

---

## 結論與對策

雖然搭載 GMS 的現代 Android 手機在 WebView 行為上大體一致，但受到 **原生 `shouldOverrideUrlLoading` 不支援 POST**、**第三方超級 App 內核**、**無 GMS 設備**、**OEM 廠商魔改限制** 以及 **第三方 App 拒絕實作 `WebChromeClient`** 的多重夾擊，單純依賴前端原生彈窗 (`window.open` 或 `<a target="_blank">`) 的穩定性極度脆弱。
