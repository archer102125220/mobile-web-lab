...
└── navigator-language/       # [Lab 3] Cross-platform language & locale detection
```
...
| **`navigator-language`** | 🤖 Android<br>🍏 iOS<br>🌐 Web | • System locale vs. Browser app locale vs. WebView locale<br>• `navigator.language` & `navigator.languages` return order<br>• HTTP `Accept-Language` weighted parsing & RFC 4647 negotiation<br>• ECMAScript 402 `Intl` API cross-platform compatibility suite | [Go to Module ➔](./navigator-language/README_en.md) |
...
### 3. Run `navigator-language`
```bash
# Start Node.js Web Server (with auto LAN discovery & native config injection)
...
# Open Android Project (Android Studio)
# Open the navigator-language/Android folder
```
